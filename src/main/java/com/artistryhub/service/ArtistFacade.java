...
public class ArtistFacade {
	...
	public List<Artist> getAll(){
		...
		List<Artist> result = DAOManager.readAll();
		return result;
	}
	...
	public Artist create(Artist newArtist) {
		...
		if (newArtist.getName().length() > MIN_NAME) {
			if (DAOManager.read(newArtist.getName()) != null) {
		...
		if (newArtist.getId() > 0) {
			if (DAOManager.read(newArtist.getId()) != null) {
	...
	public Artist delete(Object key) {
	    ...
	    if (key instanceof Artist) {
	        ...
	        deletedArtist = DAOManager.read(((Artist) key).getId());
	    } else {
	  
	        deletedArtist = DAOManager.read(key);
	    }
	    ...
	    DAOManager.delete(deletedArtist);
	    DAO.commit();
