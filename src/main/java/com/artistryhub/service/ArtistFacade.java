...
public class ArtistFacade {
    ...
    protected DAOArtist DAOMainManager = new DAOArtist();
    protected DAOPresentation DAOPresentation = new DAOPresentation();
    protected DAOCity DAOCity = new DAOCity();
    ...
    public Artist create(String name, ArrayList<String> type, String biography) {
        ...
        if (DAOMainManager.read(name) != null) {
            throw new CustomException("Uniqueness violated, the id or name must be unique", ExceptionCode.UNIQUENESS_VIOLATION);
        }
    ...
    public Artist delete(Object key) {
        ...
        ArrayList<Presentation> artistApresentations = deletedArtist.getPresentations();
        
        for(Presentation artistApresentation: artistApresentations) {
        	City cityRemovedPresentation = artistApresentation.getCity();
        	
        	cityRemovedPresentation.removePresentation(artistApresentation);
        	deletedArtist.removePresentation(artistApresentation);
        	
        	DAOCity.update(cityRemovedPresentation);
        	DAOPresentation.delete(artistApresentation);
        }
    ...
    public void validateName(String name) {
        if (name == null || !Pattern.matches("^[a-zA-Z ]{4,32}$", name)) {
            throw new CustomException("Name must contain only letters and spaces, and be between " + ArtistFacade.MIN_NAME + " and " + ArtistFacade.MAX_NAME + " characters.", ExceptionCode.INVALID_NAME);
        }
    }
    ...
    public void validateType(ArrayList<String> types) {
        if (types == null || types.size() > 4) {
            throw new CustomException("Types must be at most " + ArtistFacade.MAX_TYPES + ".", ExceptionCode.INVALID_TYPE);
        }
    }
    ...
    public void validateBiography(String biography) {
        if (biography == null || biography.length() < 16 || biography.length() > 255) {
            throw new CustomException("Biography must be between " + ArtistFacade.MIN_BIOGRAPHY + " and " + ArtistFacade.MAX_BIOGRAPHY + " characters.", ExceptionCode.INVALID_BIOGRAPHY);
        }
    }
